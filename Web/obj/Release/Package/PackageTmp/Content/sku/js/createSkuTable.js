...
function setSkuVals() {
    ...
    if (b) {//必选的SKU属性已经都选中了
        ...
        //			});
        //			
        SKUTableDom += "<table class='skuTable' id='cusTable'><tr>";
        //创建表头
        for (var t = 0; t < skuTypeArr.length; t++) {
            SKUTableDom += '<th name="' + skuTypeArr[t].skuTypeTitle + '">' + skuTypeArr[t].skuTypeTitle + '</th>';
        }
        SKUTableDom += '<th name="价格">价格</th><th name="库存">库存</th>';
        SKUTableDom += "</tr>";
        //循环处理表体
        for (var i = 0; i < totalRow; i++) {//总共需要创建多少行
            ...
            for (var j = 0; j < skuTypeArr.length; j++) {//sku列
                ...
                if (0 == (i % anInterBankNum)) {//需要创建td
                    currRowDoms += '<td rowspan=' + anInterBankNum + '><input type="text" class="setting_sku_name noBorder" value="' + skuValues[point].skuValueTitle + '" readonly="readonly" josnval="" /></td>';
            ...
            SKUTableDom += '<tr propvalids=\'' + propvalids + '\' propids=\'' + propIdArr.toString() + '\' propvalnames=\'' +
                ...
                '<td><input type="text" class="setting_sku_price" value="' + alreadySetSkuPrice + '" josnval="" /></td>' +
                '<td><input type="text" class="setting_sku_stock" value="' + alreadySetSkuStock + '" josnval="" /></td>' +
                '</tr > '
                ;
        }
        SKUTableDom += "</table>";
 ...
 * 获取已经设置的SKU值 
 */
function getSkuVals(json) {
    alreadySetSkuVals = {};
    //获取设置的SKU属性值
    $("tr[class*='sku_table_tr']").each(function (index, item) {
        var skuName = $(this).find("input[type='text'][class*='setting_sku_name']").val();//SKU属性
        ...
        if (json) {
            ...
            alreadySetSkuVals[propvalids] = {
                "skuName": skuName,
                "skuPrice": json[index].Price,
                "skuStock": json[index].Inventory
            }
        }
    });
}
